...
class ScheduleMetrics extends Component {
    ...
    return (
      <div className="scheduleMetricsContainer">
        ...
        <p>
          ...
          <b>
            {calcTotalHours(
              convertAppointmentsListToSingleHours(this.props.appointments),
              "week"
            )}
          </b>
        </p>
        <p>
          Number of open hours this month:{" "}
          <b>{calcTotalHours(this.props.availableHours, "month")}</b>
          <br />
        ...
        <p>
          Number of open hours (all time):{" "}
          <b>{this.props.availableHours.length}</b>
          <br />
  ...
  createChartData() {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      availableHoursPerMonth[i] = calcTotalHoursPerMonth(
        this.props.availableHours,
        i
      );
      appointmentHoursPerMonth[i] = calcTotalHoursPerMonth(
        convertAppointmentsListToSingleHours(this.props.appointments),
        i
      );
    }
